...
const handleAddDevice = async () => {
  ...
  try {
    ...
    console.log('AddUpdateDevice response:', result);
    if (!result.isError) {
      Alert.alert('Success', 'Device added successfully.');
      navigation.navigate('Home');
    } else {
      const backendMessage = result.responseException?.exceptionMessage || result.message || 'Failed to add a Device.';
      Alert.alert('Error', backendMessage);
    }
  } catch (e) {
    console.log('Add Device Error:', e);
    Alert.alert('Error', 'Something went wrong while adding the device. Please try again.');
  }
};
