...
const Sensorcontrol = ({ route }) => {
    ...
    const fetchComponentDetails = async () => {
        try {
            const apiUrl = `http://moonhub.moonpreneur.com/LMSService/api/IOTDevice/GetComponentStatus?device_component_id=${component.device_component_id}&device_token=${device.device_token}`;
            ...
            <View style={{ flex: 1 }}>
                <View style={{ flexDirection: 'row', width: '100%', paddingHorizontal: 20, marginTop: 20, justifyContent: 'space-between' }}>
                    <TouchableOpacity onPress={() => navigation.goBack()} style={styles.crossButton}>
                        <Image source={require('../assets/assets/images/A-1.png')} style={{ width: 20, height: 15 }} />
                    </TouchableOpacity>
                </View>
